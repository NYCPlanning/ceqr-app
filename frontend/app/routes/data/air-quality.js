...
export default class DataAirQualityRoute extends Route {
  async model() {
    ...
    const depFile = await fetch(
      `${dataDir}dep_cats_permits/${versionTxt}`,
      fetchOptions
    );
    const nysdecFile = await fetch(
      `${dataDir}nysdec_state_facility_permits/${versionTxt}`,
      fetchOptions
    );
    const titleVFile = await fetch(
      `${dataDir}nysdec_title_v_facility_permits/${versionTxt}`,
      fetchOptions
    );
