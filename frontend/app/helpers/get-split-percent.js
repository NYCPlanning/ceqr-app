...
export function getSplitPercent(params /* , hash */) {
  ...
  if (modalSplitData && variables && Array.isArray(variables)) {
    ...
    const partTotal = variables.reduce(
      (runningSum, variable) =>
        modalSplitData[variable]
          ? runningSum + modalSplitData[variable].value
          : runningSum,
      0
    );
    // calculate percent
    const percent = modalSplitData.trans_commuter_total
      ? (partTotal / modalSplitData.trans_commuter_total.value) * 100
      : undefined;
