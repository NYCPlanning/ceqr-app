...
export function getAggregatePercent(params/* , hash */) {
  ...
  if (allModalSplitData && Array.isArray(allModalSplitData) && variables && Array.isArray(variables)) {
    ...
    const partTotal = allModalSplitData.reduce((runningSum, modalSplit) => runningSum + variables.reduce((runningSum, variable) => (modalSplit[variable] ? runningSum + modalSplit[variable].value : runningSum), 0), 0);
