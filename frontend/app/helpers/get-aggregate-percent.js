...
export function getAggregatePercent(params /* , hash */) {
  ...
  if (
    allModalSplitData &&
    Array.isArray(allModalSplitData) &&
    variables &&
    Array.isArray(variables)
  ) {
    // calculate the aggregate of the sums of given variables for all modal splits
    const partTotal = allModalSplitData.reduce(
      (runningSum, modalSplit) =>
        runningSum +
        variables.reduce(
          (runningSum, variable) =>
            modalSplit[variable]
              ? runningSum + modalSplit[variable].value
              : runningSum,
          0
        ),
      0
    ...
    const wholeTotal = allModalSplitData.reduce(
      (runningSum, modalSplit) =>
        ...
        modalSplit.trans_commuter_total
          ? runningSum + modalSplit.trans_commuter_total.value
          : runningSum,
      0
    );
