...
module('Unit | Fragment | SubdistrictTotals', function(hooks) {
  ...
  test('#buildings returns correct filters list', function(assert) {
    ...
    let project_with_only_hs = SubdistrictTotals.create({
      allBuildings: schools,
      level: 'hs',
    });

    assert.deepEqual(project_with_only_hs.buildings, []); // 365 Question: could also check that this does not exist if we end up not returning an empty array
    ...
    let ps_school_totals = SubdistrictTotals.create({
      allBuildings: schools,
      level: 'ps',
      district: 1,
      subdistrict: 2
