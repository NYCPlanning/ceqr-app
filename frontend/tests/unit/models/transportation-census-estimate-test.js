...
import { VARIABLE_MODE_LOOKUP } from 'labs-ceqr/models/census-tract';
...
module('Unit | Model | transportation census estimate', function(hooks) {
  ...
  test('it returns correct display mode from variable', function(assert) {
    ...
    let trans_total = store.createRecord('census-tract', { variable: 'trans_total' });
    let trans_auto_total = store.createRecord('census-tract', { variable: 'trans_auto_total' });
    let trans_public_total = store.createRecord('census-tract', { variable: 'trans_public_total' });
