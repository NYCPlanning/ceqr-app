...
module('Unit | Model | transportation analysis', function(hooks) {
  ...
  test('it correctly calculates square feet, ratios, and sum of ratios for project land use', async function(assert) {
    let analysisMirage = server.create('transportation-analysis', hasAllLandUses);
  ...
  test('it correctly calculates detailedAnalysis', async function(assert) {
    ...
    let analysisMirageFastFood = server.create('transportation-analysis', {
          trafficeZone: 2,
          project: association({
            totalUnits: 2,
            commercialLandUse: [
    ...
    let analysisMirageCommunityFacility = server.create('transportation-analysis', {
        trafficeZone: 2,
        project: association({
    ...
    let analysisMiragesumOfRatiosOver1 = server.create('transportation-analysis', {
        trafficeZone: 2,
        project: association({
          totalUnits: 2, // residentialUnitsRatio = residentialUnits / thresholdFor residentialUnits zone 2 = 2/200 = 0.01
          parkingLandUse: [
    ...
    let analysisMirageNoConditions = server.create('transportation-analysis', {
        trafficeZone: 2,
        project: association({
          totalUnits: 2,
          commercialLandUse: [
    ...
    let projectFastFood = await this.owner.lookup('service:store').findRecord(
      'project', analysisMirageFastFood.projectId, { include: 'transportation-analysis' }
    );
    // project with community facility
    let projectCommunityFacility = await this.owner.lookup('service:store').findRecord(
      'project', analysisMirageCommunityFacility.projectId, { include: 'transportation-analysis' }
    );
    // project with sumOfRatiosOver1
    let projectsumOfRatiosOver1 = await this.owner.lookup('service:store').findRecord(
    ...
    let projectNoConditions = await this.owner.lookup('service:store').findRecord(
      'project', analysisMirageNoConditions.projectId, { include: 'transportation-analysis' }
    );

    let analysisFastFood = await projectFastFood.get('transportationAnalysis');
    let analysisCommunityFacility = await projectCommunityFacility.get('transportationAnalysis');
    let analysisSumOfRatiosOver1 = await projectsumOfRatiosOver1.get('transportationAnalysis');
    let analysisNoConditions = await projectNoConditions.get('transportationAnalysis');
