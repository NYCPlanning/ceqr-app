...
module(
  'Integration | Component | transportation/census-tracts-table',
  function (hooks) {
    ...
    skip('it displays a table with columns and subcolumns for all selected census tracts', async function (assert) {
      ...
      assert.equal(
        tableHead.querySelectorAll(':scope > tr:first-child > th').length,
        this.stubSelectedCensusTractIds.length + 2
      );
      ...
      assert.equal(
        tableHead.querySelectorAll(':scope > tr:nth-child(2) > th').length,
        2 * this.stubSelectedCensusTractIds.length + 2
      );
    });
    ...
    skip('it displays table with rows for transportation modes, each row displaying a count and a percent for each census tract column', async function (assert) {
      ...
      for (let i = 0; i < table.rows.length; i++) {
        ...
        if (i < table.rows.length - 5) {
          assert.ok(
            modeLabelCell.textContent.includes(
              VARIABLE_MODE_LOOKUP[COMMUTER_VARIABLES[i]]
            )
          );
        }
        ...
        if (i === table.rows.length - 2) {
          ...
          const percentCellFormat =
            i === table.rows.length - 1 ? /-/ : /\d+\.\d\s%/;
