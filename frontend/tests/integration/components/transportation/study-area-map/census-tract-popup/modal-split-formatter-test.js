...
module('Integration | Component | transportation/study-area-map/census-tract-popup/modal-split-formatter', function(hooks) {
  ...
  test('it properly displays population and modal split data', async function(assert) {
    ...
    this.server.create('census-tract', {geoid: '1', variable: 'population', value: population });
    const variable = 'variable';
    const value = 10;
    const moe = 1;
    this.server.create('census-tract', {geoid: '1', variable, value, moe });

    // When those census tract records are passed into the component
    this.data = await this.owner.lookup('service:store').query('census-tract', { geoid: '1'});
    await render(hbs`
      {{transportation/study-area-map/census-tract-popup/modal-split data=data}}
    `);
