...
module(
  'Integration | Component | transportation/tdf/modal-splits',
  function (hooks) {
    ...
    test('temporal split totals calculate correctly when user manually inputs mode split values', async function (assert) {
      ...
      const project = await store.findRecord('project', 1, {
        include:
          'transportation-analysis,transportation-analysis.transportation-planning-factors',
      });

      // replicating how availablePackages is defined on routes/project/show/transportation/tdf/planning-factors/show.js
      const dataPackage = project.transportationAnalysis
        .get('transportationPlanningFactors')
        .firstObject.get('dataPackage');
      ...
      this.transportationPlanningFactorsResidentialModel =
        project.transportationAnalysis.get(
          'transportationPlanningFactors'
        ).firstObject;
    ...
    `);
      ...
      assert.equal(
        this.element.querySelector('[data-test-total-am]').textContent,
        '0',
        'total am default'
      );
      assert.equal(
        this.element.querySelector('[data-test-total-md]').textContent,
        '0',
        'total md default'
      );
      assert.equal(
        this.element.querySelector('[data-test-total-pm]').textContent,
        '0',
        'total pm default'
      );
      assert.equal(
        this.element.querySelector('[data-test-total-saturday]').textContent,
        '0',
        'total saturday default'
      );
    ...
    `);
      ...
      assert.ok(
        this.element
          .querySelector('[data-test-total-all-periods]')
          .textContent.includes('0'),
        'total all periods default'
      );
    ...
    `);
      ...
      assert.notOk(
        this.element.querySelector('[data-test-column-title="all periods"]')
      );
    ...
    `);
      ...
      assert.ok(
        this.element.querySelector('[data-test-modal-split-input-am="auto"]')
      );
      assert.notOk(
        this.element.querySelector(
          '[data-test-all-period-percent-value="auto"]'
        )
      );
    ...
    test('user can toggle Edit Mode and add/remove modes from the displayed rows', async function (assert) {
      ...
      this.server.create('project', {
        publicSchoolsAnalysis: this.server.create('publicSchoolsAnalysis'),
        communityFacilitiesAnalysis: this.server.create(
      ...
      this.project = project;
      this.analysis = await project.get('transportationAnalysis');
      this.transportationPlanningFactorsResidentialModel = project
        .get('transportationAnalysis')
        .get('transportationPlanningFactors').firstObject;
      this.availablePackages = availablePackages;
    ...
    `);
      ...
      assert.ok(
        this.element.querySelector('[data-test-row-mode-title="taxi"]'),
        'row title includes taxi before edit'
      );
      assert.notOk(
        this.element.querySelector('[data-test-row-mode-title="ferry"]'),
        'row title does not include ferry before edit'
      );
      ...
      assert.ok(
        this.element.querySelector('[data-test-active-mode="taxi"]'),
        'taxi in active mode'
      );
      ...
      assert.ok(
        this.element.querySelector('[data-test-inactive-mode="taxi"]'),
        'taxi in inactive mode'
      );

      // ##### ADD AN ITEM TO THE ACTIVE LIST ##################################
      // check that the ferry mode item is now displayed in the inactive modes section
      assert.ok(
        this.element.querySelector('[data-test-inactive-mode="ferry"]'),
        'ferry in inactive mode'
      );
      ...
      assert.ok(
        this.element.querySelector('[data-test-active-mode="ferry"]'),
        'ferry in active mode'
      );
      ...
      assert.ok(
        this.element.querySelector('[data-test-row-mode-title="ferry"]'),
        'row title includes ferry after edit'
      );
      assert.notOk(
        this.element.querySelector('[data-test-row-mode-title="taxi"]'),
        'row title does not include taxi after edit'
      );
    });
    ...
    `);
      ...
      assert.notOk(
        this.element.querySelector('[data-test-census-tract="205"]')
      );
    });
  }
);
