...
module(
  'Integration | Component | transportation/tdf/modal-splits/census-tracts-map/features',
  function (hooks) {
    ...
    test('it has default legend and unchecked boxed when flags are flase', async function (assert) {
      ...
      await render(
        hbs`{{transportation/tdf/modal-splits/census-tracts-map/features }}`
      );
    ...
    test('it has populated legend and checked boxed when flags are true', async function (assert) {
      ...
      // When the component is rendered
      await render(
        hbs`{{transportation/tdf/modal-splits/census-tracts-map/features showLandUse=showLU showTransitZones=showTZ}}`
      );
      ...
      assert.equal(
        findAll('[data-test-tz-legend-item]').length,
        Object.keys(transitZoneColors).length
      );
      assert.equal(
        findAll('[data-test-lu-legend-item]').length,
        Object.keys(landUseColors).length
      );
    });
  }
);
