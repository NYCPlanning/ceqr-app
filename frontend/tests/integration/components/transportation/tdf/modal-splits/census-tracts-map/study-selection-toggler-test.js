...
module(
  'Integration | Component | transportation/census-tracts-map/study-selection-toggler',
  function (hooks) {
    ...
    `);
      const geoid = '1';
      this.set('selectedFeatures', [{ properties: { geoid } }]);
      await settled();
    ...
    skip('it removes a selected feature geoId from the analysis model study selection', async function (assert) {
      ...
      const geoid = 'geoid';
      this.model.set('transportationAnalysis.jtwStudySelection', [
        geoid,
        '1',
        '2',
      ]);
