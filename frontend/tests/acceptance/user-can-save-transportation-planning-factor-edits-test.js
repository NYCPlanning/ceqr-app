...
module(
  'Acceptance | user can save transportation planning factor edits',
  function (hooks) {
    ...
    test('User can save changes made to transportation planning factors', async function (assert) {
      ...
      assert.equal(
        this.element.querySelector('[data-test-total-per="residential"]')
          .textContent,
        'Total (Per 1 DU)'
      );
      ...
      assert.equal(
        this.server.db.transportationPlanningFactors.firstObject.landUse,
        'residential'
      );
      // checking that last object in factors has landUse "office"
      assert.equal(
        this.server.db.transportationPlanningFactors.lastObject.landUse,
        'office'
      );
      ...
      assert.equal(
        this.server.db.transportationPlanningFactors.lastObject.inOutSplits.am
          .in,
        50
      );
      ...
      assert.equal(
        this.element.querySelector('[data-test-total-per="office"]')
          .textContent,
        'Total (Per 1000 sq ft)'
      );
      ...
      assert.equal(
        this.server.db.transportationPlanningFactors.firstObject.inOutSplits.am
          .in,
        13
      );
      // "out" value should be 100 minus "in" value
    ...
    test('User can save changes made to transportation Trip Results notes', async function (assert) {
      ...
      assert.equal(
        this.element.querySelector('[data-test-text="table note"]').textContent,
        'peaches & cream'
      );

      assert.equal(
        this.server.db.transportationPlanningFactors.firstObject.tableNotes
          .personTrips,
        'peaches & cream'
      );
    });
